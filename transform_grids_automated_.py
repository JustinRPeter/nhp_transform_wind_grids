...
@elapsed(logger,success)
def transform_downwards(period, gcm, rcp, **kwargs):
    ...
    var_map = {var_name: join(ipath,var_name+"_day_" + gcm + "_" + rcp + "_r1i1p1_AWAP_[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9].nc4")}
    ...
    for gcm in listGCM:
        for rcp in listRCP:
            if rcp == 'historical':
                ...
                period = pd.date_range("1 jan 2006","31 dec 2099", freq='D')

            if gcm == "ACCESS1-0" and rcp == "historical":
                print("Skipping ACCESS1-0 - historical")
                break # historical for access has already been run

            if gcm == "CCAM-r3355-ACCESS1-0" and rcp == "historical":
                print("Skipping CCAM ACCESS - Historical")
                break # historical CCAM ACCESS has been ran

            print("starting " + gcm + " " + rcp)
                # Thing to keep in mind here, is that file paths might be off due to lftp mirror not keeping file structure from gadi -> ap2
            
            args = dict(
                in_path='/data/cwd_awra_data/AWRAMSI/IWRM_0042_WP3/GIT/Jack/py3/wind/transform-2m-grids-to-10m/input/' + gcm + "/AWAP/",
                out_path='/data/cwd_awra_data/AWRAMSI/IWRM_0042_WP3/GIT/Jack/py3/wind/transform-2m-grids-to-10m/output/' + gcm + "/" + rcp + "/",
                h5_grids='/data/cwd_awra_data/AWRAMSI/IWRM_0042_WP3/GIT/Stuart/py3/wind/transform-2m-grids-to-10m/davenport-vertical-wind-profile-parameters-0.05-mean.h5'
            )

            transform_downwards(period, gcm, rcp, **args)

            #call transform_downwards(period)
